...
def find_default_elf() -> Path | None:
    # Template cores use *_core.elf; Doom links as doom.out.
    candidates = sorted(SCRIPT_DIR.glob("*_core.elf")) + sorted(
        SCRIPT_DIR.glob("doom.out")
    )
    if len(candidates) == 1:
        return candidates[0]
    if len(candidates) > 1:
...
def main() -> None:
    ...
    parser.add_argument(
        "--elf",
        type=Path,
        help="ELF with debug symbols (default: sole *_core.elf / doom.out next to this script)",
    )
    parser.add_argument(
        "--tool",
    ...
    if elf is None:
        raise SystemExit("no ELF found; pass --elf path/to/doom.out (or *_core.elf)")
    if not elf.is_file():

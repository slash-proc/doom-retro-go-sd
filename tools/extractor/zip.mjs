...
//
// Names may also contain "/". A core's install is two trees rather than one:
// the core itself goes to cores/doom.bin and what the converter produces is a
// game, so it goes to roms/doom/*.whd where the launcher browses. A flat zip
// would hand the user a pile of files and a layout to work out themselves. Every segment is still validated: no empty segment, no "..", no
